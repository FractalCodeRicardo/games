...
function Score:draw_flags()
    local row_start = 120
    for id, score in pairs(self.player_flags) do
        local player = Player.get_by_id(id)
        local title = player.name .. " score:"

        self:draw_flag(row_start, title, score)
        row_start = row_start + 40
    end
end

function Score:draw_flag(y, text, score)
...
function Score:update_score(score)
    ...
    self.player_flags = score.player_scores
end
