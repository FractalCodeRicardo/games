...
local function get_color(player_id)
    if player_id == nil then
        return gfx.COLOR_DARK_PURPLE
    end

    -- player_id is just the index of the player
...
function Board:iterate(callback)
    for y = 1, SIZE do
        for x = 1, SIZE do
            local cell = self.cells[y][x]
            callback(cell)
...
function Board:get_scores()
    local player_scores = {}
    local opens = 0
    local flags = 0
    local player_open_mine = nil

    self:iterate(function(cell)
        local player_id = cell.open_by
        ...
        if player_scores[player_id] == nil then
            player_scores[player_id] = 0
        end

        if cell.open and cell.mine then
            player_scores[player_id] = 0
            player_open_mine = cell.open_by
        end

        if cell.flag and cell.mine then
    ...
    return {
        ...
        player_open_mine = player_open_mine 
    }
end

function Board:get_winner()
    local all_scores = self:get_scores()

    local scores = all_scores.player_scores
    local winner = nil
    local score = 0
